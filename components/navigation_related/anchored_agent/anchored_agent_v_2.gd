extends Node
class_name AnchoredAgentV2 # using Navigator API
...
func move_via_input(actor: Node2D, delta: float, dir: Vector2) -> void:
	...
	var edge_length: float = navigator.get_len(current_edge_ids[0], current_edge_ids[1])
	...
	while distance > 0:
		...
		edge_length = navigator.get_len(current_edge_ids[0], current_edge_ids[1])
		progress += distance / edge_length
		distance = (progress - 1) * edge_length
...
func _update_last_direction() -> void:
	var point1: Vector2 = navigator.astar.get_point_position(current_edge_ids[0])
	var point2: Vector2 = navigator.astar.get_point_position(current_edge_ids[1])
...
func _align_current_edge_with_input(direction: Vector2) -> bool:
	...
	var edge_vector: Vector2 = (navigator.get_pos(current_edge_ids[1]) - navigator.get_pos(current_edge_ids[0])).normalized()
	var dot: float = 0.0
...
func _get_best_new_edge(point_id: int, dir: Vector2) -> int:
	var cons := navigator.astar.get_point_connections(point_id)
	var pos: Vector2 = navigator.astar.get_point_position(point_id)
	var best_id: int = -1
	var best_score: float = -INF
	for con in cons:
		if con == current_edge_ids[0]:
			continue
		if navigator.get_point_data(con).get_layer_id() != navigator.get_point_data(current_edge_ids[1]).get_layer_id():
			# Block input movement to current layer when using free move (arrow keys)
			continue
		
		var score: float = (navigator.astar.get_point_position(con) - (pos)).normalized().dot(dir)
		if score > best_score:
			best_score = score
			best_id = con
	return best_id
